...
sap.ui.define([
	...
	return BaseController.extend("psu.isr.Isr.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Zz1Isrno", FilterOperator.Contains, sQuery)];
			} else {
		...
		_onMasterMatched: function () {
			this._oListSelector.oWhenListLoadingIsDone.then(
				function (mParams) {
					...
					if (mParams.firstListitem.getBindingContext().getProperty("Zz1Role") === "R") {
						this.getModel("appView").setProperty("/addEnabled", true);
					}
